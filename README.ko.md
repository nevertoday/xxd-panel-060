...
# 🦁 XXD Panel 060｜선(禪) 미니멀 시

### 사진을 1~3개의 검은 핵심 형태로 응축해 고요한 울림이 남는 시각 철학으로
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 060 예시 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 060 예시 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 060 예시 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 060 예시 4"></a></td>
  </tr>
</table>

> 원본 게시물의 네 번째 미디어 슬롯은 세 번째와 동일합니다. 저장소는 두 첨부 슬롯을 원문 그대로 보존하지만 서로 다른 디자인으로 소개하지 않습니다.
...
<table>
  ...
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 060 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 060 예시 8"></td>
  </tr>
</table>

## 3:4 세로형 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 060의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 060 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 060 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 060 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 060 추가 상하 예시 4"></td>
  </tr>
</table>

> [샤오샤오둥（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2092190049999937946) · 2026년 8월 25일<br>
> GPT2 × 선의 정취 × 미니멀리즘 × 미학 프롬프트 × VOL.060

<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946">원본 예시 게시물 보기 →</a></p>

이 예시는 060의 미학적 결과만 보여 줍니다. 예시의 주제, 구성, 색상, 문구, 이전 캔버스 비율은 새 작업의 참고나 기본값이 되지 않습니다.
...
**Panel 060**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 1~3개의 검은 핵심 형태로 응축해 고요한 울림이 남는 시각 철학으로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
## 빠른 판단: XXD Panel 060이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 1~3개의 검은 핵심 형태로 응축해 고요한 울림이 남는 시각 철학으로 |
| **한눈에 알아볼 특징** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-060`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-060-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-060-prompt.zh-CN.md)
...
검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편

사진을 가장 중요한 하나의 형태, 기억할 하나의 관계, 의미 있는 하나의 시각 명제로 압축합니다. 검은 주체는 망점, 줄무늬, 물결선, 입자와 소멸 속에서 동시에 나타나고 사라지며, 매우 옅은 종이 바탕과 거대한 여백은 고요함, 고독, 시간, 바라봄 자체를 화면의 내용으로 만듭니다.


## 입력을 고유한 완성작으로 바꾸는 방식

사진에서 가장 식별하기 쉬운 주체, 윤곽, 자세, 공간 관계와 감정적 은유를 정한 뒤 장면 전체를 의도적으로 버립니다. 중요한 형태 하나, 관계 하나, 시각 명제 하나만 남겨 원래 대상의 출처를 느끼게 하면서 고요함, 고독, 시간, 바라봄과 존재를 더 깊이 읽게 합니다.

```text
정체성·윤곽·자세·공간 관계 고정 → 숨은 뜻과 존재 명제 읽기 → 장면 전체와 사실적 세부 삭제 → 1~3개의 검은 핵심 형태로 응축 → 망점·그리드·물결선·입자·어긋남·소멸로 나타남／사라짐 형성 → 매우 옅은 종이와 거대한 여백으로 정신성 구성 → 방백·개념 메모·생각의 파편 통합
```

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- 일반적으로 핵심 형태 1~3개만 남기며 윤곽, 실루엣, 물결, 산, 원, 눈, 길, 끊어진 경계는 원본의 숨은 뜻에서 나옵니다.
- 검정을 절대적인 주인공으로 삼고 망점, 그리드, 줄무늬, 물결선, 입자, 어긋남, 반복, 희석과 국소 왜곡으로 형태를 만듭니다.
- 경계는 선명하거나 점차 풀리고 흐려지고 끊어져 나타남과 사라짐이 함께 존재할 수 있습니다.
- 미색, 안개 회색, 옅은 황토, 분홍 회색, 매우 옅은 청회색 종이 바탕이 빈 공간에 정신성과 호흡을 부여합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 060의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-060 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-060 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-060 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-060 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-060 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-060 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 060 적용, 공통 설정은 한 번만 확정
/xxd-panel-060 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
