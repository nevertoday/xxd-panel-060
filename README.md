...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 060 样张 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 060 样张 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 060 样张 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 060 样张 4"></a></td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 060 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 060 样张 8"></td>
  </tr>
</table>
...
<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2092190049999937946">查看原样张推文 →</a></p>

这些样张只展示 060 的审美结果；样张主体、构图、配色、文字和旧画幅不会成为新任务的参考或默认值。
...
**Panel 060** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用 1–3 个黑色核心形体，把照片提炼成安静而有回声的视觉哲思**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
> 用 XXD Panel 060 帮我处理这张图，先推荐最合适的构图和尺寸。
...
黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片

它把照片压缩到一个最关键的形、一段最值得记住的关系和一个最有意味的视觉命题。黑色主体在网点、条纹、波线、颗粒与消散中同时“显现”和“消失”，极浅纸面与巨大留白则把安静、孤独、时间和观看本身变成画面内容。

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 060 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 用 1–3 个黑色核心形体，把照片提炼成安静而有回声的视觉哲思 |
| **一眼可辨的特点** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
这套风格先锁定照片里最有辨识度的主体、轮廓、姿态、空间关系与情绪隐喻，再主动舍弃场景全貌。它最终只保留一个关键形、一段关系和一个视觉命题，让原物仍可被感知，同时读出更深的安静、孤独、时间感、观看感或存在感。

```text
锁定身份、轮廓、姿态与空间关系 → 读出潜台词和存在命题 → 删除场景全貌与现实细节 → 凝练为 1–3 个黑色核心形体 → 用网点、栅格、波线、颗粒、错位与消散塑造显现／消失 → 以极浅纸面和巨大留白建立精神性 → 融入一句旁白、概念注记或思考碎片
```

## 成品中最容易识别的风格特点

- 通常只保留 1–3 个核心形体；轮廓、剪影、波纹、山形、圆、眼状、路径或断裂边界都来自原图潜台词。
- 黑色是绝对主角，通过网点、栅格、条纹、波线、颗粒、错位、重复、稀释与局部畸变形成形体。
- 边界可清晰，也可逐渐松散、模糊、断裂，使“显现”与“消失”同时存在。
- 背景保持米白、雾灰、浅赭、淡粉灰或极浅蓝灰等近纸面浅色，让空白承担精神性与呼吸感。
- 主体可悬置、偏心、压低、上提或缩小；极少文字像旁白或思考碎片，不解释画面。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-060-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 060 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-060 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-060 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-060 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 060；公共设置只解析一次
/xxd-panel-060 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-060`. 安装后请重启 Agent 会话。
